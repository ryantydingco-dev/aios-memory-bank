...
Merge fields: {{first_name}}, {{school_name}} (drop leads missing either).
Approved facts only. Plain text, no dashes. Emails 2-4 thread as replies, no subjects.

## Email 1 (day 0) — subject: {{school_name}}'s spirit wear store
...
Timing is the reason to look now: the school year is starting, and anything you want
in hands for fall needs about two weeks of production plus shipping. Proofs come back
in 24 to 48 hours. The store keeps working after fall, but the first order window is
right now.

Want the preview so you can start the clock?
