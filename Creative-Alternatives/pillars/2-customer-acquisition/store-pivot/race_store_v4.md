...
no contact first name). Approved facts only. Plain text, no dashes. Emails 2-4 thread
as replies, no subjects.
...
Quick math on {{race_name}}: production runs about two weeks, plus shipping. With
your race in {{race_month}}, the comfortable order window is the next few weeks, and
proofs come back in 24 to 48 hours once you pick a design. The shirts you have to
order come off the same page, so the preview doubles as your starting gun.
