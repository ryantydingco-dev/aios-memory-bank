"""
AIOS Client Workspace — Collection Orchestrator

Discovers and runs all active collectors (collect_*.py files in this directory).
After collection, regenerates key-metrics.md so /prime always has fresh data.

Usage:
    python scripts/collect.py                          # Run all collectors
    python scripts/collect.py --sources smartlead      # Run specific ones
...
def discover_collectors():
    ...
    for filepath in sorted(SCRIPT_DIR.glob("collect_*.py")):
        ...
        if filepath.name == "collect.py":
            continue
        # Extract source name: collect_smartlead.py -> smartlead
...
def main():
    ...
    # Exit 1 only on real errors — an all-skipped run (fresh machine, no creds) is not a failure
