...
## 2026-08-11 maximum-profit doctrine + Pillar-3 GTM system built + campaigns at scale

- **Maximum-Profit Plan created (`plans/ca-maximum-profit-plan-2026-08-11.md`) — new operating doctrine.** Objective: maximize **collected gross profit + durable enterprise value**, not sends/traffic/reply counts. "CA does not need another growth brainstorm — the constraint is converting known demand into quotes, paid orders, repeat orders, and protected margin." 30-day operating order: (1) collect cash already earned → (2) close opportunities already in motion → (3) call high-value lapsed customers → (4) protect margin on every quote → (5) work the two live deadline-driven outbound lanes → (6) only then add the next lane. North-star scoreboard: cash collected, collected GP, open quote value (owner + dated next action on every quote), quote turnaround (same business day where possible).
- **Event Merch Planning Center built (Codex, morning voice-session delegation) — Pillar 3's first real system.** Complete review-ready inbound + GTM package at `pillars/3-online-presence/inbound/event-merch-planning-center/` (16 files: GTM narrative, one funnel, "Build My Booth-Ready Merch Plan" CTA, attribution ID `event_merch_plan`, content engine, trust/proof system, measurement, 30-day + LinkedIn launch queues) plus edits across the pillar-3 inbound implementation files. Inbound explicitly wired to the live fall exhibitor outbound motion. **Nothing published or live; sits in Codex worktree `492c` (detached), awaiting Ryan review + merge decision. Kenny approval flagged on every public promise.** Source-of-truth tension surfaced: older camps/web-store strategy files vs the current trade-show/event motion — resolved as scoped campaign direction, not an offer rewrite.
- **Campaigns at scale.** Trade-show campaign 3777819 topped up overnight (25 email_finder jobs drained + 5 failed lanes recovered, 2,311 ids resubmitted): **3,575 → 5,385 leads, ACTIVE.** Three more campaigns activated in the morning (~20.1k new leads into motion): Q4 Gifting (10,827, cap 1000/day), Law Firm Admins (9,109, cap 1000/day), Galas (194, cap 200/day). Winter Shows 2027 (4,603) deliberately held DRAFTED "[starts November]" per the 8-12wk window rule. Replies now arriving daily; the daily-swag-engine loop fulfilled Impact Canopy (SEMA) same-day and a second trade-show reply in the evening.
- **LinkedIn engine running but gated.** Daily scheduled run fires (drives Ryan's Chrome), but all 3,281 prospects remain in TARGET — `templates_approved: false` and `booking_link_confirmed: false` in config. Ryan approving templates + confirming the booking link is the unblock.
- **Infrastructure:** memory-bank repo cleaned + republished for the Mac Studio clone (.gitignore hardened, `Creative-Alternatives/.venv/` untracked — 3,603 files; commit `3db14ec` pushed). Studio clone is go. daily-wf-brief's `history.json` append is fixed (08-07 + 08-08 landed; WF balance $468,729.78, +$24.6K/2wk). Reference asset: Smartlead capability map (pricing/features verified Aug 2026) completed for plan-fit decisions.
