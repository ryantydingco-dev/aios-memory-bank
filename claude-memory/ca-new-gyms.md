...
**Not built:** the find→Claude-refine→enrich→AI Ark chain isn't a single command (Claude refinement is the in-session step); no weekly cron yet (could mirror ca_signals_weekly). Regex name-extraction stays coarse by design. Related: [[ca-tradeshow-signal]], [[ca-buying-signals]], [[creative-alternatives-aios]].
